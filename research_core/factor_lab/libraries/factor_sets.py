...
from research_core.factor_lab.libraries.arxiv_paper import IMPLEMENTED_ARXIV_FACTORS, arxiv_specs, compute_arxiv_factors
...
def compute_factor_set(df: pd.DataFrame, factor_set: str, factor_names: list[str] | None = None) -> pd.DataFrame:
    ...
    if normalized in {"arxiv", "arxiv_paper", "paper"}:
        return compute_arxiv_factors(df, factor_names=factor_names)
