...
- `research_core/agents/factor_research_agent.py` — 因子研究智能体
- `research_core/agents/factor_scanner_agent.py` — 因子扫描智能体
- `research_core/agents/cross_validation_agent.py` — 交叉验证智能体
- `research_core/agents/model_inference_agent.py` — 模型推理智能体
- `research_core/qlib_lab/auto_factor_miner.py` — 自动因子挖掘
...
- 智能体层已初步实现，支持因子发现、扫描、验证和推理。
- 前端已接入智能体运行入口，可触发智能体执行并展示结果。
- 后续需完善与 gate 引擎的集成，实现完整的自动化工作流。

### L4: 服务注册与任务编排层

当前已有：

- `research_core/registry/service_registry.py` — 服务注册与发现
- `research_core/registry/task_orchestrator.py` — 任务编排与调度

目标职责：

- 管理系统中的各类服务（因子服务、模型服务、智能体服务）。
- 支持服务注册、发现和健康检查。
- 任务编排与调度，支持异步执行和进度追踪。
...
- `research_core/kanban/kanban_engine.py` — 看板核心引擎
- `research_core/kanban/metric_calculator.py` — 指标计算器（IC/IR/风险指标）
- `research_core/kanban/classification.py` — 因子分类管理

目标职责：

- 因子可视化与统计分析。
- 计算 IC、Rank IC、ICIR、风险等指标。
- 因子分类管理与看板构建。
