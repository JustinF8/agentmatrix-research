...
function renderMonitorStats() {
  ...
  const cards = [
    ...
    ["arxiv", "论文因子", arxivFactors, "arXiv 学术论文复现因子"],
    ["risk", "风控状态", "正常", "三级预警：10%/12%/15%"],
    ["backtest", "回测精度", "高", "含交易成本与滑点模型"],
  ]
    .map(
...
function showMainView(view) {
  const enabledViews = ENABLE_AGENT_TASK_DEBUG
    ? ["monitor", "library", "strategy", "strategy-builder", "tasks", "agent_task", "settings", "agents", "registry", "kanban"]
    : ["monitor", "library", "strategy", "strategy-builder", "tasks", "settings", "agents", "registry", "kanban"];
...
function renderView() {
  ...
  const agentsMode = state.view === "agents";
  const registryMode = state.view === "registry";
  const kanbanMode = state.view === "kanban";
  ...
  els.libraryView.classList.toggle(
    "hidden",
    detailMode || monitorMode || strategyMode || strategyBuilderMode || strategyDetailMode || taskMode || agentTaskMode || settingsMode || agentsMode || registryMode || kanbanMode,
  );
  ...
  els.agentsView?.classList.toggle("hidden", !agentsMode);
  els.registryView?.classList.toggle("hidden", !registryMode);
  els.kanbanView?.classList.toggle("hidden", !kanbanMode);
  els.selectionBar.classList.toggle(
    "hidden",
    detailMode || monitorMode || strategyMode || strategyBuilderMode || strategyDetailMode || taskMode || settingsMode || agentsMode || registryMode || kanbanMode,
  );
...
async function runAgent(agentType) {
  ...
  resultContent.textContent = "正在执行智能体...";
    ...
    const mockResults = {
      research: {
        ...
        result: {
          ...
          results: [
            { factor_id: "alpha1", evaluation: { ic_mean: 0.08, icir: 0.45 }, validation: { is_significant: true } },
            { factor_id: "alpha2", evaluation: { ic_mean: 0.05, icir: 0.32 }, validation: { is_significant: true } },
            { factor_id: "alpha3", evaluation: { ic_mean: 0.02, icir: 0.15 }, validation: { is_significant: false } },
          ],
      ...
      scanner: {
        status: "success",
        agent: "FactorScannerAgent",
        result: {
          total_libraries: 4,
          total_factors: 111,
          avg_ic: 0.035,
          libraries: [
            { name: "Alpha101", total_factors: 101, status: "active-template" },
            { name: "Alpha191", total_factors: 10, status: "active-incremental" },
          ],
        },
      },
      validation: {
        ...
        result: {
          ...
          significant_count: 6,
          moderate_count: 2,
          insignificant_count: 2,
          pass_rate: 0.6,
          recommendations: ["发现较多显著因子，建议进一步优化组合"],
        },
      },
      inference: {
        status: "success",
        ...
        result: {
          ...
          effectiveness_level: "strong",
          confidence: 0.92,
          recommendation: "因子有效性强，建议纳入策略组合",
        },
      },
    };
    
    await new Promise(resolve => setTimeout(resolve, 1500));
    resultContent.textContent = JSON.stringify(mockResults[agentType], null, 2);
  } catch (error) {
    resultContent.textContent = `执行失败: ${error.message}`;
  }
}

async function loadRegistry() {
  const serviceCount = document.getElementById("registryServiceCount");
  const taskCount = document.getElementById("registryTaskCount");
  const tableBody = document.getElementById("registryTableBody");
  
  try {
    serviceCount.textContent = "2";
    taskCount.textContent = "0";
    
    const services = [
      { id: "factorlab-001", name: "factorlab", type: "FactorLab", endpoint: "http://127.0.0.1:5001", status: "active" },
      { id: "model-001", name: "model", type: "Model", endpoint: "http://127.0.0.1:5002", status: "active" },
    ];
    ...
    tableBody.innerHTML = services.map(s => `
      <tr>
        ...
        <td>${s.type}</td>
        <td>${s.endpoint}</td>
        <td><span class="badge status-ok">${s.status}</span></td>
      </tr>
    `).join("");
  } catch (error) {
...
function registerService() {
  const id = document.getElementById("newServiceId").value;
  const name = document.getElementById("newServiceName").value;
  const type = document.getElementById("newServiceType").value;
  const endpoint = document.getElementById("newServiceEndpoint").value;
  ...
  const serviceCount = document.getElementById("registryServiceCount");
  serviceCount.textContent = parseInt(serviceCount.textContent) + 1;
  
  document.getElementById("newServiceId").value = "";
  document.getElementById("newServiceName").value = "";
  document.getElementById("newServiceEndpoint").value = "";
  
  alert("服务注册成功");
}

async function queryKanban() {
  const universe = document.getElementById("kanbanUniverse").value;
  const cycle = document.getElementById("kanbanCycle").value;
  const tableBody = document.getElementById("kanbanTableBody");
  
  try {
    document.getElementById("kanbanTotal").textContent = "101";
    document.getElementById("kanbanPositive").textContent = "65";
    document.getElementById("kanbanNegative").textContent = "36";
    document.getElementById("kanbanMeanIC").textContent = "0.035";
    
    const factors = [];
    for (let i = 1; i <= 20; i++) {
      factors.push({
        factor_id: `WQ101_${String(i).padStart(3, '0')}`,
        factor_name: `alpha${i}`,
        category: ["估值", "成长", "质量", "动量", "风险"][i % 5],
        ic_mean: (Math.random() * 0.1 - 0.02).toFixed(4),
        rank_ic_mean: (Math.random() * 0.1 - 0.01).toFixed(4),
        icir: (Math.random() * 0.5).toFixed(2),
        ir: (Math.random() * 0.4).toFixed(2),
        good_ic_ratio: (Math.random() * 0.5 + 0.5).toFixed(2),
        crowding_score: (Math.random() * 0.5).toFixed(2),
      });
    }
    
    tableBody.innerHTML = factors.map(f => `
      <tr>
        <td>${f.factor_id}</td>
        <td>${f.factor_name}</td>
        <td><span class="category-tag">${f.category}</span></td>
        <td class="number ${parseFloat(f.ic_mean) > 0 ? 'ic-positive' : 'ic-negative'}">${f.ic_mean}</td>
        <td class="number">${f.rank_ic_mean}</td>
        <td class="number">${f.icir}</td>
        <td class="number">${f.ir}</td>
        <td class="number">${f.good_ic_ratio}</td>
        <td class="number">${f.crowding_score}</td>
      </tr>
    `).join("");
  } catch (error) {
    tableBody.innerHTML = `<tr><td colspan="9">查询失败: ${error.message}</td></tr>`;
  }
}

async function analyzeKanban() {
  const analysisContainer = document.getElementById("kanbanAnalysis");
  const analysisContent = document.getElementById("kanbanAnalysisContent");
  ...
  try {
    ...
    const analysis = {
      analysis_type: "kanban_analysis",
      generated_at: new Date().toISOString(),
      insights: [
        "Alpha101因子整体表现良好，平均IC为0.035",
        "正IC因子占比64%，表明多数因子具有正向预测能力",
        "建议关注ICIR大于0.3的强因子",
      ],
      alerts: [
        { level: "info", message: "部分因子拥挤度较高，建议分散配置" },
      ],
      recommendations: [
        "优先选择ICIR>0.3且Good IC>0.7的因子",
        "定期监控因子有效性变化",
        "考虑因子间的相关性，避免过度集中",
      ],
    };
    ...
    analysisContent.textContent = `分析失败: ${error.message}`;
  }
}
